...
| `provides` | no | What the module registers, by kind: `weapons`, `perks`, `gobblegums`, `powerups`, `equipment`, `localize`, `soundbanks`, `scripts`, `models`, `effects`, each a list of names. Two modules providing the same name is a decision (below). A seed's manifest fills this in; a declaration may narrow it, never contradict it |
