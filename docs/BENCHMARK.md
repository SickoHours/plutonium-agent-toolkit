...
source and target.
The knowledge routes are inert by default; with `--output <new dir>` they also write the answer and a
receipt like any job, and these two prompts ask for that so the scorer can prove the classification
came from the toolkit (the recorded `answer.json` is hash-checked against the receipt). Artifacts are scored only through the compile receipt's recorded outputs, hash-checked,
