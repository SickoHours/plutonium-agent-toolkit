...
| `fake_linker.py` | Reads `zone_source/<zone>.zone`, packs listed rawfiles from `raw/` into `<name>.ff`, where `<name>` comes from the zone's `> name,` line as with the real Linker |
| `fake_unlinker.py` | `--list` prints the inventory; `--output-folder` extracts rawfiles; fails on a malformed fastfile; refuses a fastfile whose file name differs from its zone name, because a real T6 fastfile is bound to its name |
