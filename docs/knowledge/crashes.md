...
| `undefined is not an array, string, or vector` in `_visionset_mgr::monitor`, then an access violation | The same missing per-player state, read as a container instead of a boolean; the two lines are the same defect reported by two different reads | Same fix as the cast above; treat the two lines as one class when reading a crash |
