...
| --- | --- | --- |
| Another Call of Duty title (BO1, BO3, BO4, WaW, IW) | Black Ops III Steam Workshop items (mod-tools ports), UGX-Mods "full weapons" boards, GitHub repositories of cross-map weapon packs with per-weapon manifests | T7 `.ff` and `.xpak` (Workshop maps built with the public mod tools are unencrypted: scripts, names and strings read offline, structured assets from a loaded game; `bo3-workshop-formats.md`), loose `.ff` zones, per-weapon manifest folders |
...
- Source VPK and Unity bundles are readable offline with public libraries. An unencrypted T7
  Workshop fastfile gives up its scripts, asset names and strings offline, and the XPAK index its
  image and mesh names; its structured assets need the loaded-game capture
  (`docs/playbooks/inspect-a-bo3-map.md`). Count what is actually inside: meshes, bones, weight
  data, clips, textures, sounds.
