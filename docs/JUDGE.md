...
- **It sends your evidence to a third party.** `judge eval` posts each case's state to
  `https://api.typesafe.ai/v1/systemone`. Nothing else in this toolkit leaves your machine except
  pinned downloads and the local T3 Code server. What is sent is exactly the bytes in
