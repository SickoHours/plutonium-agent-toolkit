...
## When a route is unverified on your host

A row below names the host each receipt came from. A development route with no receipt for your
OS, or a row that says "unverified on Windows" or "Linux only", is neither broken nor off limits:
the code is the same on Windows and Linux and the unit suite passes on both; nobody has yet run it
with the real program on that host and committed the receipt. This does not apply to `game` and
`capture` routes off Windows, which are unsupported by transport, nor to macOS, which is untested
and unclaimed. The toolkit is built to be measured by whoever is
using it. Run [playbooks/qualify-on-this-host.md](playbooks/qualify-on-this-host.md): it produces the receipt with `tools/qualify.py`, checks the redaction, and ends
with a pull request that turns this table's claim into a fact. Where the real program then behaves
differently from the fake in `tests/fakes/`, that is a defect to fix in the adapter with a
regression, not a reason to stop.
...
| `gsc decompile` | writes-output | native | thread-1 | Linux only: real gsc-tool 1.4.10 decompiles the script Tier 2 just compiled ([receipts/0.1.0b1/linux-tier2-backends.json](receipts/0.1.0b1/linux-tier2-backends.json) step `gsc decompile the compiled script`). Semantic equivalence to the source is not checked. **Unverified on Windows: [playbooks/qualify-on-this-host.md](playbooks/qualify-on-this-host.md)** |
...
| `module plan/build` | writes-output | native | thread-1 | Linux only: `examples/hello-pack` (both example modules on the stock game) planned in dependency order and built into one `mod.ff` with real gsc-tool 1.4.10 and OpenAssetTools 0.33.0, both rawfiles read back, the receipt verified with `--inputs` and the package inspected ([receipts/0.1.0b1/linux-tier2-backends.json](receipts/0.1.0b1/linux-tier2-backends.json) steps `module plan hello-pack`, `module build hello-pack (real gsc-tool + OAT)`, `project verify --inputs the pack receipt`, `ff inspect the pack mod.ff`). Refusals for a missing dependency, a declared conflict, a cycle, a module not declared for the base or map, two modules producing the same file, an exceeded budget and malformed declarations are covered by fakes (`tests/test_compositions.py`). Scripts-only modules; no assets, loads or engine-measured limits. **Unverified on Windows: [playbooks/qualify-on-this-host.md](playbooks/qualify-on-this-host.md), Tier 2** |
| `project init` | writes-output | native | thread-1 | Linux only: `init` then `plan` of the result ([receipts/0.1.0b1/linux-tier1-offline.json](receipts/0.1.0b1/linux-tier1-offline.json) steps `project init`, `project plan the init recipe`). **Unverified on Windows: same playbook, Tier 1** |
| `model inspect`, `model convert` | writes-output | native | thread-1 | Linux only: real Blender 5.2.1 with the bundled worker and Cast 2.00 on a synthetic OBJ cube, `inspect` and `convert --format cast` ([receipts/0.1.0b1/linux-tier2-backends.json](receipts/0.1.0b1/linux-tier2-backends.json) steps `model inspect cube.obj`, `model convert cube.obj to cast`). No rig, no animation, no Cast input; in-game appearance untested; the rigged fixture in the same receipt also covers `inspect` on an armature with a skin and an action. **Unverified on Windows: run [playbooks/qualify-on-this-host.md](playbooks/qualify-on-this-host.md) with `--media` and land the receipt** |
| `model transform/rename-bones/retime/preview` | writes-output | native | thread-1 | Linux only: real Blender 5.2.1 on a two-bone rigged, skinned, animated `.blend` the installed Blender generated (`tools/qualify.py --media`): rename `tag_weapon`, retime 30 to 60 fps with the 1..10 frame range verified to become 2..20, scale 2, and a 640 px preview ([receipts/0.1.0b1/linux-tier2-backends.json](receipts/0.1.0b1/linux-tier2-backends.json) steps `model rename-bones rig.blend`, `model retime rig.blend 30 to 60 fps`, `retime doubled the frame range`, `model transform rig.blend scale 2`, `model preview rig.blend`). A glTF input refuses `transform` and `retime` by design (the importer adds NLA tracks); no Cast rig input; in-game appearance untested. **Unverified on Windows: same playbook, `--media`** |
| `audio inspect/convert` | writes-output | native | thread-1 | Linux only: real FFmpeg 9.0 ffprobe/ffmpeg on a generated 440 Hz stereo WAV, `inspect` and `convert --format wav --rate 48000 --channels 1` with the stream parameters verified by ffprobe ([receipts/0.1.0b1/linux-tier2-backends.json](receipts/0.1.0b1/linux-tier2-backends.json) steps `audio inspect tone.wav`, `audio convert tone.wav 48 kHz mono`). In-game playback untested. **Unverified on Windows: same playbook, same `--media` tier.** Fake ffmpeg/ffprobe cover parameter mismatch and no-stream input |
