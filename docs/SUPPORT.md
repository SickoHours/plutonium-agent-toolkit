...
| `dev builtin` | downloads-source | native | core | Linux receipt: `--plan` (no network), a fetch of the three built-ins from GitHub at the pinned commit (one snapshot, three receipts, a pack laid out with its members), a rerun that verifies and downloads nothing, and `module plan` of the built-in pack from the shelf ([receipts/0.1.0b1/linux-tier2-backends.json](receipts/0.1.0b1/linux-tier2-backends.json) steps `dev builtin --plan`, `dev builtin fetches the built-ins`, `dev builtin rerun verifies`, `module plan the built-in pack from the shelf`). `tests/test_builtin.py` covers the shipped registry equal to `examples/registry.json`, the reserved name, origins in list, search and show, one download per snapshot, receipts and layout, planning from the shelf, `doctor`'s block, a changed tree refused with `artifact_changed`, an unrecorded tree refused, the lock, a failed download leaving no partial shelf, and a pack recipe that escapes its snapshot. No Windows receipt yet; expected to pass there (Tier 2 of [playbooks/qualify-on-this-host.md](playbooks/qualify-on-this-host.md) records it) |
