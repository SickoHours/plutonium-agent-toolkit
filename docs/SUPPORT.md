...
| Item | Status |
| --- | --- |
| Windows 11 x64, native Python 3.11+ | Tiers 1 (offline) and 2 (real backends) passed on Windows 11 25H2, build 26200, Python 3.12.0: [receipts/0.1.0a1/tier1-offline.json](receipts/0.1.0a1/tier1-offline.json), [receipts/0.1.0a1/tier2-backends.json](receipts/0.1.0a1/tier2-backends.json). Tier 3 (running game) attempted, [receipts/0.1.0a1/tier3-game.json](receipts/0.1.0a1/tier3-game.json): `launch`, console attach, `install-mod`, `select-mod` and `quit` work natively with fresh engine replies; `load-map` from inside a running match is not yet verified (issue #8; see the `game` rows). No `game` route earned level `game`. |
| Windows 10 | untested |
...
| `game install-mod` | writes-output | native | thread-2 | file copy with hash check; refuses overwrite without `--replace`; moves the old folder aside. Ran natively twice, the second with `--replace` ([receipts/0.1.0a1/tier3-game.json](receipts/0.1.0a1/tier3-game.json) step `install-mod hello_zm receipt`). Refuses a source not named `mod.ff`: a T6 fastfile is bound to its file name |
...
| `game select-mod/reload-mod/load-map/fast-restart/map-restart/disconnect/quit` | changes-game | offline | thread-2 | `load-map town` ran natively ([receipts/0.1.0a1/tier3-game.json](receipts/0.1.0a1/tier3-game.json)) and set the UI/gametype dvars, as a cold start from the frontend, which is out of scope: the client drops such a match with `SV_Shutdown: hostquit` right after it loads (issue #8). `load-map` is for switching maps inside a match started from the Plutonium menu, and that flow has not yet been verified natively. `select-mod zm_gobblegums` and `select-mod base` ran natively with verified `fs_game` changes, and `quit` stopped the client through the engine's own `quit` (same receipt); restarts and `disconnect` were not exercised. All stay `offline`: a `game` flip needs a playable spawn (issue #8). |
