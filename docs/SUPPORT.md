...
| `registry baseline` | writes-output | native | thread-1 | Linux receipt: `examples/hello-zm` scanned with outcome `passed` ([receipts/0.1.0b1/linux-tier1-offline.json](receipts/0.1.0b1/linux-tier1-offline.json) steps `registry baseline hello-zm`, `baseline outcome for hello-zm is passed`). No Windows receipt yet; expected to pass there (Tier 1 of [playbooks/qualify-on-this-host.md](playbooks/qualify-on-this-host.md) records it). Static scan of a module or composition directory; executes nothing, no backend, no network, no model. `tests/test_baseline.py` (24 tests): both bundled examples pass; a PE header under a `.txt` name, ELF and Mach-O headers and plugin paths (`native-plugin`); `curl \| sh`, the PowerShell forms and a downloaded file started later (`download-and-execute`); a link in the tree, `..` in a recipe source and absolute paths (`path-escape`); archive URLs with and without a SHA-256 (`unpinned-acquisition`); repository, commit, missing path, empty lists and invalid JSON (`declaration-mismatch`); every capability from a minimal fixture; a >8 MiB binary; large text not scanned; an unreadable file and directory (`incomplete`); identical reports for the same tree; the file and byte bounds; the per-file row bound; the job wiring beside the non-job `registry` actions. Not measured: real community repositories. `docs/REGISTRY.md` |
