...
| `plane actions/serve` | inert / serves-local | offline | thread-4 | The control plane: `plane actions` lists the typed action table (every action one registered route, its effect, status, availability here, parameters, confirmation); `plane serve` binds 127.0.0.1 with a per-start token, lists declarations and compositions under the library roots by reading their files, and runs each action as a `pat` child with `--output` under the jobs directory. `tests/test_plane.py` (22 tests): token, `Host`, `Origin` and `Content-Length` guards; path parameters confined to their roots and file names (absolute, `..`, links, wrong root and wrong file refused); unknown, missing and mistyped parameters refused before any child starts; plan, build and verify of `examples/hello-pack` through the server against the fake backends with the receipt index; one run at a time (`busy`, with nothing written); a failed child recorded with its error; the run lock released when a record cannot be written; a running child stopped at shutdown; the catalog bound and malformed manifests reported; model slugs, option values and prompt bytes bounded exactly as `pat agent` bounds them; confirmation required for game and agent writes; Windows-only actions refused off Windows; a private seed shown as absent; the `serve` entry's root checks and deadline. The page itself was driven in headless Chromium on the authoring host (a plan whose collisions came back as a decisions list, the confirmation dialog cancelled with nothing started, `agent models` filling the instance and model choices) and not recorded as a receipt. Not measured: the game actions on Windows through the plane, and a real dispatch through the plane. `docs/CONTROL-PLANE.md` |
