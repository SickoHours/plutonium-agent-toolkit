...
A row below names the host each receipt came from. A route with no receipt for your OS, or a row
that says "unverified on Windows" or "Linux only", is neither broken nor off limits: the code is
the same on both platforms and the unit suite passes on both; nobody has yet run it with the real
program on that host and committed the receipt. The toolkit is built to be measured by whoever is
...
| `gsc decompile` | writes-output | offline | thread-1 | fake gsc-tool only; **no native run on any host**: add a decompile step to `tier_backends` in `tools/qualify.py`, then run the playbook above |
...
| `project init` | writes-output | offline | thread-1 | unit tests only; **no native run on any host**: add an init step to `tier_offline`, then run the playbook above |
...
| `model transform/rename-bones/retime/preview` | writes-output | offline | thread-1 | background Blender with the bundled worker; fake Blender in tests. **No native run on any host**: add steps for them to `tier_media` in `tools/qualify.py`, then run the playbook above |
