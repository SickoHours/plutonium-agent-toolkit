...
| `mcp tools/serve` | inert / serves-stdio | offline | thread-4 | The Model Context Protocol bridge over the control plane's runtime: `mcp tools` prints the generated tool definitions and serves nothing; `mcp serve` speaks JSON-RPC on stdin and stdout. `tests/test_mcp.py` (11 tests) covers the generated schemas (every action a tool, names the protocol accepts, `additionalProperties` false, required a subset of properties, `confirmed` required exactly for the state-changing actions, paths as the structured root-and-relative object, the effect and this host's availability in each description), the handshake (version negotiation and the fallback for an unknown one, methods gated before `initialize`, unknown method, unknown notification, malformed params), a read that runs a child and returns its document, a job that writes a receipt under the jobs directory with the run listed, refusals that start no child (unknown tool, missing and unknown parameters, a path leaving its root, a missing confirmation, an action this host cannot run), a failing child reported as its own document with its exit status, the line loop (parse error, oversized message drained, notifications unanswered, deadline), and a real child process over pipes whose stdout carries only protocol and then the invocation's own document. Not covered: a third-party MCP client's own behaviour. No Windows receipt yet; expected to pass there (Tier 1 of [playbooks/qualify-on-this-host.md](playbooks/qualify-on-this-host.md) records it) |
