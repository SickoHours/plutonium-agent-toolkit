...
| `workspace catalog` | inert | offline | core | Source-labelled records and optional HUD, per-item menu and wallbuy icon bindings; tests/test_workspace_catalog.py covers malformed shapes, registry links, snapshot hashing, per-row and per-reply budgets, and paging over a synthetic 400-module workspace. No native qualification receipt; no backend or game actions. |
...
| `module inspect` | inert | offline | thread-1 | Inert declaration-only protocol `pat.module-inspect/1`; synthetic metadata and source CLI tests, including symlinked-parent parity with plan, final-link refusal, bounded UTF-16 diagnostics and escaped-surrogate output. No native qualification receipt; payload resolution and gameplay are outside scope. When `evidence.json` sits beside a module declaration it is validated too (`tests/test_module_ledger.py`): a valid ledger is summarized, a malformed one is diagnostics on the `ledger` key with the declaration still `metadata-valid`, absence adds nothing. The declaration's promise vocabulary is validated and echoed here too (`tests/test_promises_vocabulary.py`): `replaces.files` as any relative zone path, the `exclusive` roles, `service`, and a `dependencies` entry as `{id, kind, why}`. `registration` (`self`, `entry` or `none`: who prints the module's one console line `<id> >> registered` at init, with `entry` refused without an `entry` field) is validated and echoed the same way (`tests/test_registration_line.py`). |
| `module ledger-add` | writes-record | offline | thread-1 | `tests/test_module_ledger_add.py` (26 tests): the ledger created from the declaration's id when absent, rows appended to an existing one with the file's own `ensure_ascii` kept both ways and every earlier row untouched, several `--row` files and a file of several rows in the order given, the six facts reported per appended row with a `game-tested` row's unobserved facts left `null` and a survival location kept apart from its map, and the refusals with nothing written: an invalid row (every diagnostic with its row index, JSON Pointer and source file), a non-object row, unreadable and non-JSON row files, an exact duplicate of an existing row and of another row in the same invocation, a subject that is another module's id, a ledger already on disk that does not validate, a missing declaration, the 1024-row limit counted after the write, an `at` that is not a date on the calendar, a row holding a lone surrogate, and the usage refusals; and the write itself — an `evidence.json` that is a symlink refused with its target untouched, a FIFO refused on a thread that has to finish, a directory refused, an interrupted write leaving the ledger that was there and no temporary file behind, and two appends running at once landing both rows under the ledger's advisory lock. No native receipt. The route validates and records; it verifies no record a row points at, and appending a row is not evidence that the run behind it happened. |
| `module state --ledger`, `module ledger-from-registry` | inert | offline | thread-1 | `tests/test_module_ledger.py` (25 tests): every row type of `docs/evidence-ledger.md` validated with one JSON Pointer per defect; the six facts derived per query and per scope from rows of the matching type only, unknown kept `null`, a rejection `false`, an `accepted-in-pack` row feeding nothing, a package query isolating rows; the CLI reporting, refusing composition flags with `--ledger`, and counting only valid rows; the registry proposal on a synthetic workspace shaped like the private one (verdict list, build revisions, a pack build skipped, sealed archive record, lineage parent, overlay parent, outside receipt flagged) with the workspace hashed before and after to prove nothing was written. No native receipt; the proposal was run read-only against the private workspace's real rows on the authoring host and not recorded. |
| `module accept` | writes-output | offline | thread-1 | `tests/test_module_accept.py` (25 tests): the ledger created when the module has none and appended to when it has one, the rows already there byte-for-byte unchanged, a second verdict a second row rather than an edit, every optional field (`record-sha256`, `reporter`, `quote`, `not-covered`, `note`, `at`) landing as given, the reported row equal to the written one, and the fact `module state --ledger` then derives (`true` on `accepted`, `false` on `rejected`); refusals with the file untouched for an outcome outside the two the row type knows, a record path that is absolute or climbs out of the workspace, a package that is not a SHA-256, a directory with no declaration, and a ledger whose subject is another module; the shelf serialiser's handling of an accent already in the file; and `--workspace` reporting whether the cited record resolves without refusing when it does not; and the write itself — a symlinked `evidence.json` refused with its target untouched, a FIFO refused on a thread that has to finish, a ledger past the 1 MiB limit refused with `input_limit` rather than parsed, an interrupted write leaving the verdicts that were there and no temporary file behind, an `--at` that is not a calendar date refused, and two verdicts recorded at once landing as two rows. No native receipt: the route runs no backend, so there is nothing a native host would exercise that these do not. **The route records a verdict a person gave; it neither forms nor verifies one, and a row it writes is a pointer to a record, not the record.** Writing a `player-accepted` row does not move this route, or the module, to evidence level `accepted`: that level is about the thing played, not about the file that says so. |
| `target list`, `target inspect`, `target validate` | inert | offline | thread-1 | `tests/test_targets.py`: the workspace location-table rules made public (shape, target string, survival location, six facts false, literal or expression, vector arity, unique ids, cited record listed, high confidence with file and line, literal equals cited text, no T6 claim in a note), target-file entries (parts agree with the key, kind fits the key, the id's route equals the entry's, parent listed or absent only where the map has no classic target, no duplicate ids, no two entries for one `(target, route)`, a fence citing its route script, named tables at the path their id and route give and present, `placements` counts equal to their table's rows, the file's `targets` count equal to its distinct keys), the listing from foundations plus the target file grouped by parent with `route_choices` for a location two providers ship, inspect with and without a table and the `route_choice` outcome on an ambiguous key, validate exit 1 with every diagnostic, and the `placements` field on `module.json` with the plan-time `placements:<target>` check reading the table as a hashed input and refusing an unnamed route. Fixtures copy the shapes of the workspace's real files (dual-route tables, a thin stock route registration, a start location with no parent) with synthetic values and no private paths. Run read-only against the private workspace's 42 tables, 1,553 placement rows and 46 entries on the authoring host: exit 0, no diagnostic, not recorded as a receipt. No native receipt; no coordinate is checked against a map and nothing about T6 is claimed. |
| `module plan/build/declare` | writes-output | native | thread-1 | Linux receipt: `examples/hello-pack` planned in dependency order and built into one `mod.ff` with real gsc-tool 1.4.10 and OpenAssetTools 0.33.0, both rawfiles read back, the receipt verified with `--inputs` and the package inspected; then the hello-zm package declared as a seed from its fastfile (`module declare`), composed as the base member with the second example and built against the seed, its root found in the result ([receipts/0.1.0b1/linux-tier2-backends.json](receipts/0.1.0b1/linux-tier2-backends.json) steps `module plan hello-pack`, `module build hello-pack (real gsc-tool + OAT)`, `project verify --inputs the pack receipt`, `ff inspect the pack mod.ff`, `module declare the hello-zm package`, `module plan a seed composition`, `module build a seed composition (real OAT)`). Fakes (`tests/test_compositions.py`) cover refusals (missing dependency, declared conflict, cycle, base or map not declared, two base members, nested pack for another map, nesting cycle, seed hash drift, private seed absent, budget, malformed declarations and references), collisions listed as decisions with recorded owners and identical-bytes dedupe, provides-name collisions, taxonomy validation, and seed roots and soundbanks in the built package. `tests/test_promises_planner.py` covers the two refusals a module's promises add: `ownership` (a staged base-owned path undeclared, from a listing or from the shipped per-map tables, with the owner, the evidence and the service that owns the path) and `exclusive` (two members on one role, in composition order, with its two resolutions). Not measured: asset-rich seeds with map patches or client scripts, and every engine limit. No Windows receipt yet; expected to pass there (Tier 2 of [playbooks/qualify-on-this-host.md](playbooks/qualify-on-this-host.md) records it) |
...
| `agent probe/hosts/models/dispatch/status/send/interrupt` | inert / writes-output | native | thread-3 | Linux receipt against a real T3 Code `0.0.41-nightly` server on this host, orchestration protocol 1 over HTTP ([receipts/0.1.0b1/linux-tier4-agent.json](receipts/0.1.0b1/linux-tier4-agent.json), Tier 4 of `tools/qualify.py`): probe, hosts, models, one dispatched proof thread with the caller's instance, model and `effort=low`, status until the first turn completed with the requested one-word reply, a second turn, the `busy` refusal while it ran, interrupt, and the interrupted state read back. Fakes in `tests/test_agent_routes.py` cover 401/403 mapping, validation before any request, and unknown-protocol refusal. **Protocol 2 is implemented with offline evidence**: `tests/test_agent_v2.py` and `tests/test_agent_ws_*.py` cover launch, follow-up, interrupt, projections, transport failures and bearer handling. Its native public probe on server `0.0.40` reports `drivable: true` ([agent-probe-v2-2026-09-13.json](receipts/0.1.0b1/agent-probe-v2-2026-09-13.json)); authenticated hosts/dispatch/status/send/interrupt remain unqualified because no bearer is configured. The opted-in smoke test skipped for that reason. Not covered: authenticated V2 lifecycle, attachments, approvals, questions, a remote host. No Windows receipt yet; expected to pass there (Tier 4 of the same tool). `docs/AGENT-HOSTS.md` |
...
a JSON Pointer and failed job receipt. Stitching writes test-plan.json with dependency order, merged preconditions, pairwise checks, explicit conflicts, human-only steps and coverage exclusions. The load phase also carries one log check per member whose declaration promises a registration line, and names the members that promise none under `not_covered` (`tests/test_registration_line.py`). No live qualification.
...
`module ledger-add` is offline and is the only writer of a ledger row besides `module qualify`:
it appends through the same validator, refuses a duplicate row, and writes nothing when any row
fails. The file is read and replaced under an advisory lock on a sibling `.evidence.json.lock`
and written through a temporary file that is fsynced and renamed, so two appends at once are two
rows and a failed write leaves the rows that were there; an `evidence.json` that is not a regular
file is refused before it is opened. It creates no evidence — a row is a pointer to a record, and
the record is the fact.

`module state --ledger` and `module ledger-from-registry` are offline: the ledger is validated
and its six facts derived per scope from synthetic rows shaped like the workspace's real ones;
the proposal reads a workspace and writes nothing. A ledger row is a pointer to a record and a
hash; neither route verifies the record or creates evidence.

`module accept` is offline, and it is the one route that writes a fact no tool can produce: a
person's verdict. It records what the caller was told — outcome, scope, package, record — and
verifies none of it. It does not open the cited record, check that the package was ever built or
installed, or confirm that anybody played anything; a caller that passes the wrong package hash
writes a wrong row, and the row will look exactly as valid as a right one. The validation it does
apply is the ledger's own shape, before the file is written, so a refusal leaves `evidence.json`
as it was. Treat the resulting `player_accepted` fact as the reporter's claim at that scope and
nothing wider: it says nothing about another map, another mode, co-op, or any other member of the
pack the person played.
...
refuses when one matches. Tests (`tests/test_checks.py`, `tests/test_compositions.py`) cover a
shadowing file refused and named, a clean loose path passed with the count it read, a case-mismatched
...
Pack-a-Punch and camo images) was made on the authoring host and is not checked in as a receipt.
