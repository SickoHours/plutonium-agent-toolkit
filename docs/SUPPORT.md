...
| `gsc decompile` | writes-output | offline | thread-1 | fake gsc-tool only in the committed receipts. `tools/qualify.py` Tier 2 now decompiles the script it just compiled; **the next qualification run on either host lands the receipt** ([playbooks/qualify-on-this-host.md](playbooks/qualify-on-this-host.md)) |
...
| `project init` | writes-output | offline | thread-1 | unit tests only in the committed receipts. `tools/qualify.py` Tier 1 now runs `init` and plans the result; **the next qualification run on either host lands the receipt** |
| `model inspect`, `model convert` | writes-output | native | thread-1 | Linux only: real Blender 5.2.1 with the bundled worker and Cast 2.00 on a synthetic OBJ cube, `inspect` and `convert --format cast` ([receipts/0.1.0b1/linux-tier2-backends.json](receipts/0.1.0b1/linux-tier2-backends.json) steps `model inspect cube.obj`, `model convert cube.obj to cast`). No rig, no animation, no Cast input; in-game appearance untested. **Unverified on Windows: run [playbooks/qualify-on-this-host.md](playbooks/qualify-on-this-host.md) with `--media` and land the receipt** |
| `model transform/rename-bones/retime/preview` | writes-output | offline | thread-1 | background Blender with the bundled worker; fake Blender in tests. `tools/qualify.py --media` now generates a rigged, skinned, animated `.blend` with the installed Blender and runs all four on it (probed by hand on Linux on 2026-09-10: all pass; a glTF input refuses `transform` and `retime` by design because the importer adds NLA tracks); **the next `--media` run on either host lands the receipt** |
