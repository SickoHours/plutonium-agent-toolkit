...
| `model transform/rename-bones/retime/preview` | writes-output | native | thread-1 | Linux only: real Blender 5.2.1 on a two-bone rigged, skinned, animated `.blend` the installed Blender generated (`tools/qualify.py --media`): rename `tag_weapon`, retime 30 to 60 fps with the frame range verified to double, scale 2, and a 640 px preview ([receipts/0.1.0b1/linux-tier2-backends.json](receipts/0.1.0b1/linux-tier2-backends.json) steps `model rename-bones rig.blend`, `model retime rig.blend 30 to 60 fps`, `retime doubled the frame range`, `model transform rig.blend scale 2`, `model preview rig.blend`). A glTF input refuses `transform` and `retime` by design (the importer adds NLA tracks); no Cast rig input; in-game appearance untested. **Unverified on Windows: same playbook, `--media`** |
