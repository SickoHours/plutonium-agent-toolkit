...
| Workspace (Windows, Linux) | `workspace` | A modding workspace of the person's own, outside the checkout, with an `AGENTS.md` for their agent and the layout for modules, compositions, jobs, receipts, registries and donors. [docs/GETTING-STARTED.md](docs/GETTING-STARTED.md) |
...
| Agent hosts (Windows, Linux) | `agent` | Hand a prompt to a running T3 Code server as a new thread with the caller's model and reasoning choice; read the thread back. Orchestration protocols 1 and 2; V2 writes have fake-transport coverage and await authenticated native qualification. [docs/AGENT-HOSTS.md](docs/AGENT-HOSTS.md) |
