...
- A development route with a receipt from one OS and none from the user's is expected to work
  on theirs: the code path is the same, and the upstream programs are built for Windows first, so
  Linux is the harder host and a Linux receipt is strong evidence for Windows. Run the route. If
  the real program behaves differently on this host, adapt the adapter with a test and say what
  changed; `docs/playbooks/qualify-on-this-host.md` records the receipt when it is worth having.
  Never tell the user a route "does not work here" when the truth is that nobody has recorded it
  here yet.
