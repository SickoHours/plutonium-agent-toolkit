...
AVAILABLE = [
    ...
    Route("workspace", "catalog", "Read source-labelled workspace build records, declared provides and optional icon bindings", "inert", status="implemented", owner=OWNER, notes="Reads workspace files only. No fact is inferred; no game actions.", result_protocol="pat.workspace-catalog/1"),
...
PLANNED = [
    ...
    Route("module", "verify-declaration", "Read one module's own bytes back against its declaration: every promise beside what the recipe, the seed, the source and the base's listings say",
          ...
          notes="Arguments: <module dir> [--workspace <root>] [--base-listings <dir>]... [--target <foundation>/<map>] [--strict] [--propose] --json. "
                ...
                "not_counted when the module has no page at all. Writes nothing.", result_protocol="pat.module-verify/1"),
    ...
    Route("module", "qualify", "Build one module alone on one target (declaration-blind, then qualified), verify both, and write its "
          ...
          notes="Arguments: <module dir> | --set <file>, --target <foundation>/<map>, --workspace <root>, [--member-root <dir>]..., "
                ...
                "Format: docs/MODULES.md.", result_protocol="pat.module-qualify/1"),
    Route("module", "accept", "Append one person's gameplay verdict to a module's evidence.json as a player-accepted row, "
          ...
          notes="Arguments: <module dir> --outcome accepted|rejected --base <token> --foundation <id> --map <id> --package <sha256> "
                ...
                "Format: docs/evidence-ledger.md.", result_protocol="pat.module-accept/1"),
...
register(Route("module", "changelog", "Generate one module's CHANGELOG.md from its own git history and its evidence ledger, grouped by the version each commit declared", "writes-record", status="implemented", owner=OWNER,
               notes="Arguments: <module dir> [--write | --check] --json. Protocol pat.module-changelog/1. **Only --write writes**: without it the route is inert and "
                     ...
                     "comparison as its /changelog row and never writes. Format: docs/MODULES.md.", result_protocol="pat.module-changelog/1"))
register(Route("module", "ledger-add", "Append validated rows to a module's evidence ledger: every row through the same validator module inspect applies, append-only and all-or-nothing", "writes-record", status="implemented", owner=OWNER,
               notes="Arguments: <module dir|evidence.json> --row <row.json> [--row ...] --json. Each row file holds one row object or a list of them, appended in the order given. "
                     ...
                     "receipt, no game, no network. Format: docs/evidence-ledger.md.", result_protocol="pat.module-ledger-add/1"))
register(Route("module", "ledger-from-registry", "Propose evidence.json rows for one workspace module from the registry's build_revisions and the module's docs; prints the proposal and writes nothing", "inert", status="implemented", owner=OWNER,
               notes="Arguments: <workspace> <module-id> [--dry-run] --json. Reads registry/t6-modules.json, foundations/*.json, modules/<id>/module.json, docs/ACCEPTED.json, docs/LINEAGE.json, docs/TEST.md and the archive records the registry cites. Every proposal is a dry run: the worker reviews the rows and notes, then writes modules/<id>/evidence.json itself. Format: docs/evidence-ledger.md.", result_protocol="pat.module-ledger-proposal/1"))

for route in AVAILABLE + PLANNED:
    register(route)
register(Route("work", "start", "Record one piece of work from a person's ask: what they want, the target, the donor and the door it came in by, as work.json in a new directory", "writes-output", status="implemented", owner=OWNER,
               notes="Arguments: <workspace> --title T --want TEXT --target <foundation>/<map>/<mode>[/<location>] [--donor REF --donor-kind path|release|map] [--subject <module or composition dir, relative>] [--by WHO] --output <new dir> --json. "
                     "The form door has no subject and placement decides the module; the shelf door names the module or composition the work starts from. Writes the order and a receipt; builds, plans, installs and touches nothing. Format: docs/work-orders.md."))
register(Route("work", "step", "Append one spine row to a piece of work: the step, its outcome, an optional note and the receipt that proves it", "writes-record", status="implemented", owner=OWNER,
               notes="Arguments: <work dir|work.json> --step placement|donor|build|verify|install|load|verdict --outcome started|done|failed|skipped [--note TEXT] [--receipt PATH [--workspace ROOT]] --json. "
                     "A cited receipt must exist and is hashed into the row; a row with none is recorded as narration. Locked, append-only, all-or-nothing; no job directory. Format: docs/work-orders.md."))
register(Route("work", "ask", "Record a decision the agent cannot settle alone: the question, two to four options with what each implies, and a default if any; one question waits at a time", "writes-record", status="implemented", owner=OWNER,
               notes="Arguments: <work dir|work.json> --request <request.json> --json. Refused while an earlier request is unanswered. The agent then polls work status until the answer is written. Format: docs/work-orders.md."))
register(Route("work", "answer", "Record a person's answer to one decision request, once, naming the surface it came from", "writes-record", status="implemented", owner=OWNER,
               notes="Arguments: <work dir|work.json> --request-id ID --choice KEY --by app|host|person [--note TEXT] --json. The choice is one of the request's option keys; a second answer is refused. Format: docs/work-orders.md."))
register(Route("work", "status", "Read one piece of work: the order, whether a question waits on the person, the current and next step, and every spine row with its receipt found, drifted, missing or absent", "inert", status="implemented", owner=OWNER,
               notes="Arguments: <work dir|work.json> [--workspace ROOT] --json. Re-hashes every cited receipt against the row; a row with no receipt is reported as narrated. Reads files only; never the game, a ledger or a module's six facts. Format: docs/work-orders.md.", result_protocol="pat.work-status/1"))
