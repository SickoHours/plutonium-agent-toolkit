...
PLANNED = [
    ...
    Route("module", "plan", "Resolve a composition of declared modules (dependency order, conflicts, base and map fit, "
          ...
          status="available", owner=OWNER,
          notes="Use --allow-unqualified to report base/map mismatches without refusing. Formats: docs/MODULES.md. A module.json beside each recipe or seed; a composition.json naming base, map, members (modules, nested packs, pinned references), decisions and loads. Replacement declarations are resolved here: every replaceFunc call in a module's available source must be declared under its replaces.functions, or the plan is refused; each replaces.functions and replaces.files target must be owned by exactly one member (overlapping declared replacements are refused, never decided away); entry.replace/entry.register drive the generated entry's main/init, one effective registration per target. External-symbol checks resolve each script against its own VM (.gsc server, .csc client) and fail a call needing an include or a builtin not on that VM."),
