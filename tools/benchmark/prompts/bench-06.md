...
Save the toolkit's own answer verbatim: write the stdout document of `pat knowledge signature --log <FIXTURE>/console_slice.log --json` to
`<RUN>/bench-06-classify-then-fix/knowledge-lookup.json`. The score reads that file; a fix found by guessing leaves
no such file.
