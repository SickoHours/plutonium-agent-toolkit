...
class BumpVersionTests(unittest.TestCase):
    ...
    def test_bump_promotes_unreleased_and_release_check_agrees(self):
        ...
        proc = self.run_tool("bump_version.py", "0.1.0b1", "--date", "2026-09-12")
        self.assertEqual(proc.returncode, 0, proc.stderr)
        self.assertIn("tag as v0.1.0-beta.1", proc.stdout)
        self.assertIn('__version__ = "0.1.0b1"', (self.repo / "src/plutonium_agent_toolkit/__init__.py").read_text())
        self.assertIn('version = "0.1.0b1"', (self.repo / "pyproject.toml").read_text())
        self.assertIn("`0.1.0b1`", (self.repo / "docs/SUPPORT.md").read_text().splitlines()[0])
        new_log = changelog.read_text()
        self.assertIn("## [0.1.0b1] - 2026-09-12", new_log)
        self.assertRegex(new_log, r"(?m)^## \[Unreleased\]\n\n## \[0\.1\.0b1\]", "fresh empty Unreleased section on top")
        check = self.run_tool("release_check.py", "--tag", "v0.1.0-beta.1")
    ...
    def test_bump_refuses_dirty_tree_bad_version_and_empty_unreleased(self):
        ...
        proc = self.run_tool("bump_version.py", "0.1.0b1")
        self.assertEqual(proc.returncode, 1)
