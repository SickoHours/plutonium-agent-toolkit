...
class RouteTests(unittest.TestCase):
    ...
    def test_builtin_answers_the_two_recorded_load_failures(self):
        ...
        self.assertEqual(code, 0)
        self.assertEqual(row["result"]["verdict"], "not-witnessed")
        self.assertEqual(row["result"]["also_on"], ["client"])
        # The verdict is what a caller branches on, so the inference limit sits beside it, not only in the note.
        self.assertIn("not evidence of absence", row["result"]["caution"])
        self.assertIn("witnessed on: client", row["result"]["caution"])
        self.assertNotIn("caution", invoke(["knowledge", "builtin", "setanimknob"])[1]["result"])
        ...
        self.assertEqual(row["result"]["verdict"], "not-witnessed", "a script export, not a builtin: include maps/mp/_utility")
        self.assertEqual(row["result"]["also_on"], [])
