...
- `csc-main-body:<target>`: a new per-script check that refuses a T6 client script whose `main()`
  ...
  `knowledge/crash-signatures.json` is generator output and does not carry the two rows yet; the
  maintainer's next export picks them up from the doc. Offline unit tests on Linux
