...
- `pat module plan|build --report-only loose-overrides`: record the one machine-state check (a loose global texture in `storage/t6/images` whose name the base owns) in the plan and on the receipt under `report_only_failed` instead of refusing the composition on it, the treatment `module qualify` already gives it. Only that check can be named; a check about the package itself is refused at the parser. Without the flag both routes refuse as before. Met on the first stock module built through a hand-ordered single-member composition, where `qualify` could not be used and one stale icon stopped every build.
